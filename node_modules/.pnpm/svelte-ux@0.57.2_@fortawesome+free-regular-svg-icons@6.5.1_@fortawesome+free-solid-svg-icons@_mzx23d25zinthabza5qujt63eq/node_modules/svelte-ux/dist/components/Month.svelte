...
import { startOfDay as startOfDayFunc, endOfDay as endOfDayFunc, startOfMonth as startOfMonthFunc, endOfMonth as endOfMonthFunc, format, addMonths, isSameDay, isWithinInterval, } from 'date-fns';
...
export let startOfMonth = (selected instanceof Date && startOfMonthFunc(selected)) ||
    (selected instanceof Array && selected.length && startOfMonthFunc(selected[0])) ||
    (selected &&
        hasKeyOf(selected, 'from') &&
        selected.from &&
 ...
 */
export let showOutsideDays = undefined;
/**
 * Day(s) to disable (not selectable)
 */
export let disabledDays = undefined; // TODO: Improve types - See isDayDisabled
$: isDayDisabled = (date) => {
