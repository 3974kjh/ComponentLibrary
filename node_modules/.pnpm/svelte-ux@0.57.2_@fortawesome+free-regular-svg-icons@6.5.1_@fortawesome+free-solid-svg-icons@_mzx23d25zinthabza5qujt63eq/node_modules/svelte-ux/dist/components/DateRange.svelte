<script>import { format, isAfter, isBefore, isSameDay } from 'date-fns';
import { PeriodType, DayOfWeek, getDateFuncsByPeriodType, hasDayOfWeek, replaceDayOfWeek, getPeriodTypeName, } from '../utils/date';
...
export let selected = { from: null, to: null, periodType: null };
...
$: {
    if (hasDayOfWeek(selectedPeriodType)) {
        ...
        if (selected.from && selected.to && selected.periodType) {
            const prevPeriodTypePreset = [...getPeriodTypePresets(selected.periodType)].find((x) => x.value.from &&
                isSameDay(x.value.from, selected.from) &&
                x.value.to &&
                isSameDay(x.value.to, selected.to));
            if (prevPeriodTypePreset && newPeriodType) {
                const newPeriodTypePreset = [...getPeriodTypePresets(newPeriodType)].find((x) => x.label === prevPeriodTypePreset.label);
