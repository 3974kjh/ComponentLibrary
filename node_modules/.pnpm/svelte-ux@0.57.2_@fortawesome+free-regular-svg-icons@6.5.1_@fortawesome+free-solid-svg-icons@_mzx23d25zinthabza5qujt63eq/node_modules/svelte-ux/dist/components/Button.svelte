...
import { asIconData } from '../utils/icons';
...
export let rounded = undefined; // default in reactive groupContext below
export let variant = undefined; // default in reactive groupContext below
export let size = undefined; // default in reactive groupContext below
export let color = undefined; // default in reactive groupContext below
...
$: _class = cls('Button', 'transition duration-200 ring-black/20', 'focus:outline-none focus-visible:ring-1', fullWidth ? 'flex w-full' : 'inline-flex', loading ? 'gap-2' : 'gap-1', variant === 'none' || !rounded ? '' : rounded === 'full' ? 'rounded-full' : 'rounded', variant !== 'none' && [
...
// Variant specific colors
{
    ...
    none: {},
}[variant ?? 'none']?.[color ?? 'default'], theme.root, classes?.root, $$props.class);
</script>
