...
import { autoFocus, selectOnFocus } from '../actions';
...
export let options = [];
export let optionText = (option) => option?.label ?? '';
export let optionValue = (option) => option?.value ?? null;
export let label = '';
...
export let toggleIcon = mdiChevronDown;
export let closeIcon = mdiClose;
export let activeOptionIcon = false;
...
export let fieldActions = autofocus
...
function onChange(e) {
    ...
    searchText = e.detail.inputValue;
...
function onBlur(e) {
    ...
    if (fe.relatedTarget instanceof HTMLElement &&
